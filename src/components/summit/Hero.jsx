          ...
          {(() => {
            ...
            return (
              <div style={{ display: "flex", animation: "ticker-scroll 35s linear infinite", width: "max-content" }}>
                {items.map((sponsor, i) => (
                  <div key={i} style={{ width: "280px", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 40px" }}>
                    <img
                      src={sponsor.src}
                      alt={sponsor.name}
                      style={{ height: "64px", width: "auto", objectFit: "contain", opacity: 0.45, filter: `grayscale(1)${sponsor.invert ? " invert(1)" : ""}` }}
                    />
                  </div>
                ))}
              </div>
